...
@Injectable()
export class ConnectedMLService {

    public getAllMails() {
         return DataComponents.filter(mail => mail.sent == false && mail.draft == false && mail.trash == false);
    }
   
    public getStarredMails() {
         return DataComponents.filter(mail => mail.starred == true);
    }

    public getSentMails() {
         return DataComponents.filter(mail => mail.sent == true);
    }

    public getDraftMails() {
         return DataComponents.filter(mail => mail.draft == true);
    }

    public getTrashMails() {
         return DataComponents.filter(mail => mail.trash == true);
    }

    public getMail(id: number | string) {
        return DataComponents.find(mail => mail.id === +id);
    }
}